...
func (r *RedpandaClusterReconciler) Reconcile(
	...
	var redpandaCluster corev1alpha1.RedpandaCluster
	...
	if err := r.Get(ctx, types.NamespacedName{Name: redpandaCluster.Name, Namespace: redpandaCluster.Namespace}, &svc); err != nil {
		if errors.IsNotFound(err) {
			log.V(debugLevel).Info("creating headless service")

			if err = r.createHeadlessService(ctx, &redpandaCluster); err != nil {
				log.Error(err, "Failed to create new service",
					"Service.Namespace", redpandaCluster.Namespace, "Service.Name", redpandaCluster.Name)
	...
	// nolint:dupl // first version of the Reconciliation function must be simple to review
	if err := r.Get(ctx, types.NamespacedName{Name: redpandaCluster.Name + seedSuffix, Namespace: redpandaCluster.Namespace}, &seedConfigMap); err != nil {
		if errors.IsNotFound(err) {
			...
			if err = r.createBootstrapConfigMap(ctx, &redpandaCluster); err != nil {
				log.Error(err, "Failed to create new seed redpanda ConfigMap",
					"Configmap.Namespace", redpandaCluster.Namespace, "Configmap.Name", redpandaCluster.Name+seedSuffix)
	...
	// nolint:dupl // first version of the Reconciliation function must be simple to review
	if err := r.Get(ctx, types.NamespacedName{Name: redpandaCluster.Name + seedSuffix, Namespace: redpandaCluster.Namespace}, &sts); err != nil {
		if errors.IsNotFound(err) {
...
func (r RedpandaClusterReconciler) createHeadlessService(
	ctx context.Context, clusterSpec *corev1alpha1.RedpandaCluster,
) error {
...
func (r *RedpandaClusterReconciler) createBootstrapConfigMap(
...
func copyConfig(
	c *corev1alpha1.RedpandaConfig, cfgDefaults *config.RedpandaConfig,
	...
	return config.RedpandaConfig{
		...
		AdvertisedKafkaApi: &config.SocketAddress{
			Port: c.AdvertisedKafkaAPI.Port,
		},
		KafkaApiTLS: config.ServerTLS{
			KeyFile:	c.KafkaAPITLS.KeyFile,
			CertFile:	c.KafkaAPITLS.CertFile,
			TruststoreFile:	c.KafkaAPITLS.TruststoreFile,
			Enabled:	c.KafkaAPITLS.Enabled,
		},
